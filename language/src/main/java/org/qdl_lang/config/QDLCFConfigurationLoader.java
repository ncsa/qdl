...
import static edu.uiuc.ncsa.security.core.cf.CFMultiConfigurations.*;
 ...
 */
public class QDLCFConfigurationLoader<T extends QDLEnvironment> extends CFLoggingConfigLoader<T> implements QDLConfigurationConstants, ConfigurationLoader<T> {
    public QDLCFConfigurationLoader(String cfgFile, CFNode node) {
     ...
     * @param logger
     */
    public QDLCFConfigurationLoader(String cfgFile, CFNode node, MyLoggingFacade logger) {
    ...
    protected Font getFont() {
        ...
        CFNode node = getFirstNode(cn, WS_TAG);
        CFNode fontNode = getFirstNode(node, WS_FONT_TAG);
    ...
    protected boolean getFirstBooleanValue(CFNode node, String attrib, boolean defaultValue) {
    ...
    protected List<VFSConfig> getVFSConfigs() {
        ...
        CFNode vNode = getFirstNode(cn, VIRTUAL_FILE_SYSTEMS_TAG_NAME);
        if (vNode == null) {
            return new ArrayList<>();
        }
        // need to snoop through children and create VFSEntries.
        for (CFNode kid : vNode.getChildren(VFS_TAG_NAME)) {
            ...
            switch (getFirstAttribute(kid, VFS_ATTR_TYPE)) {
                ...
                case VFS_TYPE_MYSQL:
                    ...
                    CFNode myNode = getFirstNode(kid, StorageConfigurationTags.MYSQL_STORE);
                    // stash everything into the map.
                    Map<String, String> map = vv.getConnectionParameters();
                    Map<String,String> attributes = myNode.getAttributes();
    ...
    protected List<ModuleConfig> getModuleConfigs() {
        ...
        CFNode vNode = getFirstNode(cn, MODULES_TAG_NAME);
        if (vNode == null) {
            return new ArrayList<>();
        }
    ...
    public static void main(String[] args) throws Throwable{
        ...
        String path = "/home/ncsa/dev/csd/config/qdl-cfg.xml";
        ...
        QDLEnvironment config = qdlcfConfigurationLoader.load();
